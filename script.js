...
const renderTasks = () => {
  ...
  if (categoryTasks.length === 0) {
    ...
    categoryTasks.forEach((task) => {
      ...
      assignToLabel.textContent = `${task.assignTo}`;
      div.appendChild(assignToLabel);
      /*assignToLabel.classList.add("task-details");
      dueDateLabel.classList.add("task-details");*/
      assignToLabel.classList.add("task-assign");
      dueDateLabel.classList.add("task-duedate");
...
const renderAssignToOptions = () => {
  const assignToSelector = document.getElementById("assign-to-select");
  assignToSelector.innerHTML = "";

  AssignTo.forEach((assignee) => {
    const option = document.createElement("option");
    option.value = assignee.id;
    option.textContent = assignee.assignTo;
    assignToSelector.appendChild(option);
  });
};
...
const addTask = (e) => {
  ...
  const assigneeId = document.getElementById("assign-to-select").value;
  const category = categorySelect.value;

  if (task === "") {
    alert("Please enter a task");
  } else {
    const assignee = AssignTo.find((a) => a.id === parseInt(assigneeId));

    const newTask = {
      id: tasks.length + 1,
      task,
      dueDate,
      assignTo: assignee ? assignee.assignTo : "",
      category,
      completed: false,
    };
    taskInput.value = "";
